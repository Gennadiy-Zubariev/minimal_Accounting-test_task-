"""
============================================================================
pages/opening_balances.py — СТОРІНКА "ПОЧАТКОВІ ЗАЛИШКИ"
============================================================================

ЩО ЦЕ ЗА ФАЙЛ:
    Три вкладки:
      1. Готівка — стартова сума в касі.
      2. Дебіторська заборгованість — окремий рядок на КОЖНОГО контрагента,
         який винен нам на момент старту.
      3. Кредиторська заборгованість — окремий рядок на КОЖНОГО контрагента,
         кому ми винні на момент старту.

    Один і той самий контрагент може мати обидва види залишків —
    і дебіторку, і кредиторку (якщо він одночасно наш клієнт і постачальник).

ВАЖЛИВО про збереження:
    Форми записують дані ТІЛЬКИ при кліку по кнопці "Зберегти".
    Enter у полях не викликає збереження (свідоме рішення — уникаємо
    випадкових записів і конфлікту між формами на вкладках).
============================================================================
"""
...
def render():
    """Точка входу. Викликається з main.py."""
...
def _render_summary():
    """Метрики зверху — щоб видно було, що вже введено."""
...
def _opening_cash(opening_date):
    """Одне поле суми. Збереження — тільки по кнопці."""
...
def _opening_receivable(opening_date):
    """Вибір будь-якого контрагента + сума його боргу нам."""
...
# ---------------------------------------------------------------------------
# Вкладка 3: КРЕДИТОРКА
# ---------------------------------------------------------------------------
def _opening_payable(opening_date):
    """Вибір будь-якого контрагента + сума нашого боргу йому."""
...
def _clear_keys(keys: list):
    """Видаляє ключі зі session_state, щоб поля скинулись після rerun."""
    for k in keys:
